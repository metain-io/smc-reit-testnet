...
module.exports = async function () {  
  const NFT_ID = 1;
  const amount = argv.amount || 10000;
  const share = argv.share || 500;
  ...
  await NFTContract.fundLiquidationVault(NFT_ID, ethers.utils.parseEther(amount.toString()));
  await NFTContractForCreator.unlockLiquidationPerShare(NFT_ID, ethers.utils.parseEther(share.toString()));
};
