...
describe("Buying IPO", function () {
  ...
  it("User 2: try to buy many NFT without staking", async function () {
    console.log("\x1b[33m%s\x1b[0m", `\nUser 2: try to buy many NFT without staking`);
    let error;
    try {
      // transfer USDT for User
      await USDContract.transfer(shareholder[2].address, TEST_SHARE_HOLDER_FUND);
  ...
  it("User 2: Staking => buy NFT => Admin pay/unlock Dividend => buy more NFT => Admin pay/unlock Dividend => claim Dividend", async function () {
    console.log("\x1b[33m%s\x1b[0m", `\nUser 2: Staking => buy NFT => Admin pay/unlock Dividend => buy more NFT => Admin pay/unlock Dividend => claim Dividend`);
    // transfer MEI for User
    await MEIContract.transfer(shareholder[2].address, TEST_SHARE_HOLDER_FUND);

    // allow NFTContract get MEI from user
    await MEIContractForShareholder[2].increaseAllowance(NFTContract.address, TEST_SHARE_HOLDER_FUND);

    let meiBalance = ethers.utils.formatEther(await MEIContract.balanceOf(shareholder[2].address));
    console.log(`User 2: MEI balance before staking: ${meiBalance} MEI`);

    // staking will level 1
    await NFTContractForShareholder[2].stake(LOYALTY.Condition[1]);

    meiBalance = ethers.utils.formatEther(await MEIContract.balanceOf(shareholder[2].address));
    console.log(`User 2: MEI balance after staking: ${meiBalance} MEI`);
    ...
    const getDividendPerShare = ethers.utils.formatEther(await NFTContractForCreator.getDividendPerShare(NFT_ID, 0));
    console.log(`User 2: getDividendPerShare: ${getDividendPerShare} USD`);

    // User get Dividends info of user
    const shareholderDividend = ethers.utils.formatEther(await NFTContractForShareholder[2].getTotalClaimableDividends(NFT_ID));
    console.log(`User 2: Dividends info: ${shareholderDividend} USD`);

    // buy NFT with USDT again
    await IPOContractForShareholder[2].purchaseWithToken("USDT", NFT_ID, TEST_SHARES_TO_BUY_2 / 2);
    ...
    const newShareholderDividend = ethers.utils.formatEther(await NFTContractForShareholder[2].getTotalClaimableDividends(NFT_ID));
    console.log(`User 2: new Dividends info: ${newShareholderDividend} USD`);

    const usdBalance1 = ethers.utils.formatEther(await USDContract.balanceOf(shareholder[2].address));
    console.log(`User 2: USD balance before claim: ${usdBalance1} USD`);

    // user claim dividend money
    await NFTContractForShareholder[2].claimDividends(NFT_ID);

    const usdBalance2 = ethers.utils.formatEther(await USDContract.balanceOf(shareholder[2].address));
    console.log(`User 2: USD balance after claim: ${usdBalance2} USD`);

    const claimCount = usdBalance2 - usdBalance1;
    console.log(`User 2: USD claimed: ${claimCount}`);

    expect(parseInt(claimCount)).equal(parseInt(newShareholderDividend));
  });
  ...
  it("User 3: KYC => buy NFT => transfer NFT to User 4 => register => Admin pay/unlock Dividend => check Dividend info", async function () {
    ...
    let USDbalance_user4 = ethers.utils.formatEther(await USDContract.balanceOf(shareholder[4].address));
    console.log(`User 4: USD balance before redeemLockedBalances: ${USDbalance_user4}`);

    // register NFT after transfer
    await NFTContractForShareholder[4].redeemLockedBalances(NFT_ID);

    USDbalance_user4 = ethers.utils.formatEther(await USDContract.balanceOf(shareholder[4].address));
    console.log(`User 4: USD balance after redeemLockedBalances: ${USDbalance_user4}`);
    ...
    const shareholderDividend_user3 = ethers.utils.formatEther(await NFTContractForShareholder[3].getTotalClaimableDividends(NFT_ID));
    console.log(`User 3: Dividends info: ${shareholderDividend_user3} USD`);
    const shareholderDividend_user4 = ethers.utils.formatEther(await NFTContractForShareholder[4].getTotalClaimableDividends(NFT_ID));
    console.log(`User 4: Dividends info: ${shareholderDividend_user4} USD`);

    expect(shareholderDividend_user4).not.equal(0);
  });

  it("User 5: KYC => stacking => buy NFT => Unlock Dividend 1 => buy more NFT => transfer NFT to User 6 => Unlock Dividend 2 => register => check Dividend info => claim Dividend", async function () {
    console.log("\x1b[33m%s\x1b[0m", `\nUser 5: KYC => stacking => buy NFT => Unlock Dividend 1 => buy more NFT => transfer NFT to User 6 => Unlock Dividend 2 => register => check Dividend info => claim Dividend`);
    ...
    await MEIContract.transfer(shareholder[5].address, TEST_SHARE_HOLDER_FUND);
    ...
    await MEIContractForShareholder[5].increaseAllowance(NFTContract.address, TEST_SHARE_HOLDER_FUND);
    ...
    // staking will level 1
    await NFTContractForShareholder[5].stake(LOYALTY.Condition[1]);
    ...
    let getTotalClaimableDividends_user5 = ethers.utils.formatEther(await NFTContractForShareholder[5].getTotalClaimableDividends(NFT_ID));
    ...
    let getTotalClaimableDividends_user6 = ethers.utils.formatEther(await NFTContractForShareholder[6].getTotalClaimableDividends(NFT_ID));
    console.log(`User 6: getTotalClaimableDividends after transfer: ${getTotalClaimableDividends_user6} USD`);
    let getLockedDividends_user6 = ethers.utils.formatEther(await NFTContractForShareholder[6].getLockedDividends(NFT_ID));
    console.log(`User 6: getLockedDividends_user6 after transfer: ${getLockedDividends_user6} USD`);

    let USDbalance_user6 = ethers.utils.formatEther(await USDContract.balanceOf(shareholder[6].address));
    console.log(`User 6: USD balance before redeemLockedBalances: ${USDbalance_user6}`);

    // register NFT after transfer
    await NFTContractForShareholder[6].redeemLockedBalances(NFT_ID);

    USDbalance_user6 = ethers.utils.formatEther(await USDContract.balanceOf(shareholder[6].address));
    console.log(`User 6: USD balance after redeemLockedBalances: ${USDbalance_user6}`);
    ...
    getTotalClaimableDividends_user6 = ethers.utils.formatEther(await NFTContractForShareholder[6].getTotalClaimableDividends(NFT_ID));
    console.log(`User 6: getTotalClaimableDividends after register: ${getTotalClaimableDividends_user6} USD`);
    getLockedDividends_user6 = ethers.utils.formatEther(await NFTContractForShareholder[6].getLockedDividends(NFT_ID));
    console.log(`User 6: getLockedDividends_user6 after register: ${getLockedDividends_user6} USD`);

    // USD balance before claim
    const usdBalance_user5 = ethers.utils.formatEther(await USDContract.balanceOf(shareholder[5].address));
    const usdBalance_user6 = ethers.utils.formatEther(await USDContract.balanceOf(shareholder[6].address));
    ...
    const usdBalance_user5_claimed = ethers.utils.formatEther(await USDContract.balanceOf(shareholder[5].address));
    const usdBalance_user6_claimed = ethers.utils.formatEther(await USDContract.balanceOf(shareholder[6].address));
    ...
    const claimCount_user5 = usdBalance_user5_claimed - usdBalance_user5;
    const claimCount_user6 = usdBalance_user6_claimed - usdBalance_user6;
    ...
    let getClaimedDividends_user5 = ethers.utils.formatEther(await NFTContractForShareholder[5].getClaimedDividends(NFT_ID));
    console.log(`User 5: getClaimedDividends: ${getClaimedDividends_user5} USD`);
    let getClaimedDividends_user6 = ethers.utils.formatEther(await NFTContractForShareholder[6].getClaimedDividends(NFT_ID));
  ...
  it("User 7: buy NFT => try to claim Liquidations before Admin unlock", async function () {
    console.log("\x1b[33m%s\x1b[0m", `\nUser 7: buy NFT => try to claim Liquidations before Admin unlock`);
    let error;
    try {
      // transfer USDT/MEI for User
      await USDContract.transfer(shareholder[7].address, TEST_SHARE_HOLDER_FUND);
      await MEIContract.transfer(shareholder[7].address, TEST_SHARE_HOLDER_FUND);
      // allow IPOContract get USDT/MEI from user
      await USDContractForShareholder[7].increaseAllowance(IPOContract.address, TEST_SHARE_HOLDER_FUND);
      await MEIContractForShareholder[7].increaseAllowance(NFTContract.address, TEST_SHARE_HOLDER_FUND);
  ...
  it("User 7: staking => buy NFT => Unlock Liquidations => claim Dividend", async function () {
    console.log("\x1b[33m%s\x1b[0m", `\nUser 7: staking => buy NFT => Unlock Liquidations => claim Dividend`);
    // stake will level 1
    await NFTContractForShareholder[7].stake(LOYALTY.Condition[1]);
    ...
    getClaimableLiquidations = ethers.utils.formatEther(await NFTContractForShareholder[7].getClaimableLiquidations(NFT_ID));
    console.log(`User 7: getClaimableLiquidations after Unlock: ${getClaimableLiquidations} USD`);

    const usdBalance_before = ethers.utils.formatEther(await USDContract.balanceOf(shareholder[7].address));
    console.log(`User 7: USD balance before claim: ${usdBalance_before} USD`);

    // user claim Liquidations money
    await NFTContractForShareholder[7].claimLiquidations(NFT_ID);

    const usdBalance_after = ethers.utils.formatEther(await USDContract.balanceOf(shareholder[7].address));
    ...
    let getClaimedLiquidations = ethers.utils.formatEther(await NFTContractForShareholder[7].getClaimedLiquidations(NFT_ID));
